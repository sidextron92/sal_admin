...
2. `Promise.all` — RPC snapshot + RPC trend + period expenses query + trend-window expenses query (4 parallel calls).
3. Aggregate period expenses by `function_name` in-process → compute waterfall margins.
4. Build monthly expense map keyed by `"YYYY-MM-01"` from trend-window expenses → join with trend rows by `month_start` → compute `cm1`, `cm2`, `cm3`, `ebitda` per month.
5. Return full `PnLResponse` with `revenue`, `expenses`, `rto`, `trend`, `meta`.

**Trend array shape per month:** `{ month_label, net_revenue, cm1, cm2, cm3, ebitda }`. No `cogs` or `gross_profit` in client payload.
